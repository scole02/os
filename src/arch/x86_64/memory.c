...
void _refill_pool() // called when linked list is emptied, adds PGFRM_LL_SIZE pages to pool
{
    ...
    if((uint64_t*)free_mem_pool_head == (uint64_t*)PHYS_MEMORY_UPBOUND)
    {
        printk("\nERROR: PHYSICAL MEMORY POOL EXHAUSTED\n");
        return;
    }    
    else if((uint64_t*)((void*)free_mem_pool_head + (PGFRM_LL_SIZE*PAGE_SIZE)) >= (uint64_t*)PHYS_MEMORY_UPBOUND)
    {
        // set pool size to whatever is left 
        free_mem_pool_size = (((uint64_t)PHYS_MEMORY_UPBOUND - (uint64_t)free_mem_pool_head)) / PAGE_SIZE;
        printk("INFO: last refilling of pool has %d pages\n", free_mem_pool_size);
    }
    else free_mem_pool_size = PGFRM_LL_SIZE; 

    // find the next free PAGE_SIZE aligned address, the start of free memory
    for(void * cur_pg=(void*)free_mem_pool_head; 
        cur_pg < (void*)free_mem_pool_head + (free_mem_pool_size*PAGE_SIZE); 
        cur_pg += PAGE_SIZE)
    {
